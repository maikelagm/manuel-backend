...
available_tests = [SecurityTestPost(
    name="password-strength-test",
    description="Test database password strength"
)]


class DBSecurityTestService:
    def insert_test_in_db(self, db: Session):
        inserted = crud_security_test.get_multi(db=db)
        if len(inserted) == 0:
            for test in available_tests:
                crud_security_test.create(db=db, obj_in=test)
...
db_security_test_service = DBSecurityTestService()
